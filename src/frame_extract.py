...
def print_progress(iteration, total, prefix='', suffix='', decimals=3, bar_length=100):
    ...
    percents = format_str.format(100 * (iteration / float(max(total, 1))))  # calculate the % done
    filled_length = int(round(bar_length * iteration / float(max(total,1))))  # calculate the filled bar length
...
def video_to_frames(video_path, frames_dir, overwrite=False, every=1, chunk_size=1000):
    ...
    frame_chunks[-1][-1] = min(frame_chunks[-1][-1], total)  # make sure last chunk has correct end frame, also handles case chunk_size < total
